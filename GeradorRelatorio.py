import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
from io import BytesIO

# --- CONFIGURAÇÃO DA PÁGINA ---
st.set_page_config(page_title="Gestão de Produção & Qualidade", layout="wide")
TEMPLATE_GRAFICO = "plotly_white"

# --- CSS PARA IMPRESSÃO (AJUSTADO PARA CABER TUDO) ---
st.markdown("""
    <style>
        @media print {
            /* Configuração da folha */
            @page { 
                size: landscape; 
                margin: 0.5cm; 
            }
            
            /* Esconder elementos de interface do Streamlit */
            [data-testid="stSidebar"], 
            header, 
            footer, 
            [data-testid="stToolbar"], 
            .stAppHeader, 
            .stDeployButton { 
                display: none !important; 
            }

            /* Ajustes do corpo para caber na pagina */
            body {
                zoom: 65%; /* Reduz a escala para caber mais conteúdo */
                -webkit-print-color-adjust: exact !important; /* Força impressão de cores de fundo */
                print-color-adjust: exact !important;
            }

            /* Forçar que o conteúdo flua livremente (sem scroll escondido) */
            .stApp {
                position: absolute;
                top: 0;
                left: 0;
                width: 100%;
                height: auto !important;
                overflow: visible !important;
            }

            .main .block-container {
                max-width: 100% !important;
                padding: 0 !important;
                overflow: visible !important;
            }
            
            /* Garantir que gráficos não sejam cortados */
            .js-plotly-plot {
                max-width: 100% !important;
            }
        }
    </style>
""", unsafe_allow_html=True)

st.title("🏭 Dashboard de Controle de Retidos")

# --- FUNÇÕES AUXILIARES ---
def mapear_linha(forno):
    try:
        forno_int = int(forno)
        if forno_int in [10, 11]: return 'Linha 4 e 5'
        elif forno_int in [12, 13]: return 'Linha 6'
        else: return 'Outros'
    except: return 'Outros'

def limpar_numero(val):
    if pd.isna(val): return 0.0
    if isinstance(val, (int, float)): return float(val)
    val = str(val).strip().replace('.', '').replace(',', '.')
    try: return float(val)
    except: return 0.0

@st.cache_data
def convert_df_to_excel(df):
    output = BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, index=False, sheet_name='Dados')
    return output.getvalue()

def adicionar_linha_geral(df_original, nome_linha, meta_pct):
    df_filt = df_original[df_original['Linha'] == nome_linha].copy()
    if df_filt.empty: return df_filt

    total_prod = df_filt['M2_Produzido'].sum()
    total_ret = df_filt['M2_Retido'].sum()
    
    meta_m2_total = total_prod * (meta_pct / 100)
    
    # NOVA LÓGICA: Saldo = Meta - Realizado
    saldo_total = meta_m2_total - total_ret
    
    pct_geral = (total_ret / total_prod * 100) if total_prod > 0 else 0
    
    row_geral = pd.DataFrame({
        'Linha': [nome_linha], 'Equipe': ['Média Geral'], 
        'M2_Produzido': [total_prod], 'M2_Retido': [total_ret],
        'Meta_M2': [meta_m2_total], 'Saldo_M2': [saldo_total], '% Realizado': [pct_geral]
    })
    
    df_filt['Equipe'] = df_filt['Equipe'].astype(str)
    df_final = pd.concat([df_filt, row_geral], ignore_index=True)
    df_final['Ordem'] = df_final['Equipe'].apply(lambda x: 1 if x == 'Média Geral' else 0)
    df_final = df_final.sort_values(by=['Ordem', 'Equipe'])
    return df_final

def criar_tabela_grafica(df, meta_pct):
    if df.empty: return None

    # Cor Texto %: Vermelho se > Meta
    cor_texto_pct = ['#E74C3C' if v > meta_pct else '#27AE60' for v in df['% Realizado']]
    
    # Cor Texto Saldo: Vermelho se Negativo (Estourou), Verde se Positivo (Dentro)
    cor_texto_saldo = ['#E74C3C' if v < 0 else '#27AE60' for v in df['Saldo_M2']]
    
    fmt_prod = [f"{v:,.2f}" for v in df['M2_Produzido']]
    fmt_meta = [f"{v:,.2f}" for v in df['Meta_M2']]
    fmt_ret = [f"{v:,.2f}" for v in df['M2_Retido']]
    fmt_saldo = [f"{v:,.2f}" for v in df['Saldo_M2']]
    fmt_pct = [f"{v:.2f}%" for v in df['% Realizado']]

    fig = go.Figure(data=[go.Table(
        header=dict(
            values=['<b>Linha</b>', '<b>Equipe</b>', '<b>Produção</b>', '<b>Meta (m²)</b>', '<b>Retido (m²)</b>', '<b>Saldo (Meta - Real)</b>', '<b>% Perda</b>'],
            fill_color='#2E86C1', align='center', font=dict(color='white', size=12)
        ),
        cells=dict(
            values=[df['Linha'], df['Equipe'], fmt_prod, fmt_meta, fmt_ret, fmt_saldo, fmt_pct],
            fill_color='#F7F9F9', align='center',
            font=dict(color=['black', 'black', 'black', 'black', 'black', cor_texto_saldo, cor_texto_pct], size=11),
            height=30
        )
    )])
    fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=400)
    return fig

def criar_grafico_evolucao_com_geral(df_prod, df_ret, nome_linha, meta_pct):
    df_p = df_prod[df_prod['Linha'] == nome_linha].copy()
    df_r = df_ret[df_ret['Linha'] == nome_linha].copy()
    
    if df_p.empty and df_r.empty: return None
    
    p_eq = df_p.groupby(['mes_ano', 'Equipe'])['metragem_real'].sum().reset_index().rename(columns={'metragem_real': 'M2_Produzido'})
    r_eq = df_r.groupby(['mes_ano', 'Equipe'])['m2_real'].sum().reset_index().rename(columns={'m2_real': 'M2_Retido'})
    
    if not df_p.empty:
        p_tot = df_p.groupby(['mes_ano'])['metragem_real'].sum().reset_index().rename(columns={'metragem_real': 'M2_Produzido'})
        p_tot['Equipe'] = 'Média Geral'
    else: p_tot = pd.DataFrame()

    if not df_r.empty:
        r_tot = df_r.groupby(['mes_ano'])['m2_real'].sum().reset_index().rename(columns={'m2_real': 'M2_Retido'})
        r_tot['Equipe'] = 'Média Geral'
    else: r_tot = pd.DataFrame()

    df_prod_full = pd.concat([p_eq, p_tot], ignore_index=True)
    df_ret_full = pd.concat([r_eq, r_tot], ignore_index=True)
    df_final = pd.merge(df_prod_full, df_ret_full, on=['mes_ano', 'Equipe'], how='outer').fillna(0)
    
    df_final['Meta_M2'] = df_final['M2_Produzido'] * (meta_pct / 100)
    df_final['Cor_Barra'] = df_final.apply(lambda row: '#27AE60' if row['M2_Retido'] <= row['Meta_M2'] else '#E74C3C', axis=1)
    
    df_final['Ordem_Equipe'] = df_final['Equipe'].apply(lambda x: 1 if x == 'Média Geral' else 0)
    df_final = df_final.sort_values(by=['mes_ano', 'Ordem_Equipe', 'Equipe'])
    df_final['Label_X'] = df_final['mes_ano'].astype(str) + " | " + df_final['Equipe'].astype(str)
    
    fig = go.Figure()
    
    # 1. Barras
    fig.add_trace(go.Bar(
        x=df_final['Label_X'], y=df_final['M2_Retido'], marker_color=df_final['Cor_Barra'],
        text=[f"{v:,.2f}" for v in df_final['M2_Retido']], textposition='inside', insidetextanchor='middle',
        name='Realizado (m²)', hovertemplate='<b>%{x}</b><br>Realizado: %{y:,.2f} m²<extra></extra>'
    ))
    
    # 2. Linha Meta (Traço)
    fig.add_trace(go.Scatter(
        x=df_final['Label_X'], y=df_final['Meta_M2'], mode='markers',
        marker=dict(symbol='line-ew', color='black', size=30, line=dict(width=2)),
        name=f'Meta ({meta_pct}%)', hovertemplate='Meta: %{y:,.2f} m²<extra></extra>'
    ))

    # 3. Valor Meta (Texto Colado)
    fig.add_trace(go.Scatter(
        x=df_final['Label_X'], y=df_final['Meta_M2'], mode='text',
        text=[f"{v:,.2f}" for v in df_final['Meta_M2']], textposition="top center",
        textfont=dict(size=9, color='black'), showlegend=False, hoverinfo='skip'
    ))
    
    max_val = max(df_final['M2_Retido'].max(), df_final['Meta_M2'].max()) if not df_final.empty else 100
    fig.update_layout(
        title=f"Evolução Mensal (Meta {meta_pct}%) - {nome_linha}",
        xaxis_title="Mês | Equipe", yaxis_title="m²",
        yaxis=dict(range=[0, max_val * 1.25]), template=TEMPLATE_GRAFICO,
        showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
    )
    return fig

# --- BARRA LATERAL ---
with st.sidebar:
    st.header("1. Upload de Dados")
    file_prod = st.file_uploader("📂 Arquivo de Produção", type=["xlsx", "csv"])
    file_ret = st.file_uploader("📂 Arquivo de Retidos", type=["xlsx", "csv"])
    st.markdown("---")
    
    st.header("2. Metas Gerais")
    META_PCT = st.slider("🎯 % Máximo de Perda (Geral)", 0.0, 5.0, 0.5, 0.1)
    
    st.markdown("---")
    st.header("3. Análise Específica")
    st.info("Configuração para a aba 'Análise por Motivo'")
    
# --- LÓGICA PRINCIPAL ---
if file_prod and file_ret:
    try:
        if file_prod.name.endswith('csv'): df_prod = pd.read_csv(file_prod)
        else: df_prod = pd.read_excel(file_prod)
        if file_ret.name.endswith('csv'): df_ret = pd.read_csv(file_ret)
        else: df_ret = pd.read_excel(file_ret)
    except Exception as e:
        st.error(f"Erro ao ler arquivos: {e}")
        st.stop()

    # Tratamento
    df_prod.columns = [c.lower().strip() for c in df_prod.columns]
    col_equipe_p = next((c for c in df_prod.columns if 'equipe' in c), 'equipe')
    col_forno_p = next((c for c in df_prod.columns if 'forno' in c), 'forno')
    col_metragem = next((c for c in df_prod.columns if 'metragem' in c), 'metragem')
    col_data_p = next((c for c in df_prod.columns if 'data' in c), None)

    df_prod['metragem_real'] = df_prod[col_metragem].apply(limpar_numero)
    df_prod['Linha'] = df_prod[col_forno_p].apply(mapear_linha)
    if col_data_p:
        df_prod['data_obj'] = pd.to_datetime(df_prod[col_data_p], dayfirst=True, errors='coerce')
        df_prod['mes_ano'] = df_prod['data_obj'].dt.strftime('%Y-%m')
    else: df_prod['mes_ano'] = 'Sem Data'

    df_ret.columns = [c.strip() for c in df_ret.columns]
    col_motivo = next((c for c in df_ret.columns if 'Motivo' in c), 'Motivo')
    col_m2 = next((c for c in ['M²', 'M2', 'm2'] if c in df_ret.columns), 'M²')
    col_equipe_r = next((c for c in df_ret.columns if 'Equipe' in c), 'Equipe')
    col_forno_r = next((c for c in df_ret.columns if 'Forno' in c), 'Forno')
    col_data_r = next((c for c in ['Data / Hora', 'Data', 'data'] if c in df_ret.columns), None)

    df_ret['m2_real'] = df_ret[col_m2].apply(limpar_numero)
    df_ret['Linha'] = df_ret[col_forno_r].apply(mapear_linha)
    if col_data_r:
        df_ret['data_obj'] = pd.to_datetime(df_ret[col_data_r], dayfirst=True, errors='coerce')
        df_ret['mes_ano'] = df_ret['data_obj'].dt.strftime('%Y-%m')
    else: df_ret['mes_ano'] = 'Sem Data'

    # --- CONTROLES DE ANÁLISE ESPECÍFICA (SIDEBAR) ---
    todos_motivos_brutos = sorted(df_ret[col_motivo].astype(str).unique())
    motivo_alvo = st.sidebar.selectbox("🔎 Escolha o Motivo:", ["(Selecione um motivo)"] + todos_motivos_brutos)
    
    st.sidebar.markdown("**Metas para este Motivo:**")
    
    # Meta Volume
    col_sb1, col_sb2 = st.sidebar.columns(2)
    with col_sb1:
        META_ABSOLUTA_M2 = st.number_input("M² Limite", min_value=0.0, value=100.0, step=10.0)
    with col_sb2:
        USAR_META_M2 = st.checkbox("Ativar Meta M²", value=True)
    
    # Meta Frequência
    col_sb3, col_sb4 = st.sidebar.columns(2)
    with col_sb3:
        META_FREQ_QTD = st.number_input("Qtd Limite", min_value=0, value=10, step=1)
    with col_sb4:
        USAR_META_FREQ = st.checkbox("Ativar Meta Qtd", value=False)

    # Filtros e Grupos (Geral)
    st.sidebar.markdown("---")
    st.sidebar.write("**Filtros Gerais (Aba 1)**")
    motivos_excluir = st.sidebar.multiselect("🗑️ Excluir Motivos (Geral)", options=todos_motivos_brutos)
    if motivos_excluir:
        df_ret_filtrado = df_ret[~df_ret[col_motivo].isin(motivos_excluir)].copy()
    else:
        df_ret_filtrado = df_ret.copy()

    if 'grupos_dict' not in st.session_state: st.session_state.grupos_dict = {}
    with st.sidebar.expander("➕ Criar Agrupamento (Geral)"):
        motivos_disp = sorted(df_ret_filtrado[col_motivo].unique())
        selecao = st.multiselect("Selecione:", motivos_disp)
        nome = st.text_input("Nome do Grupo")
        if st.button("Salvar Grupo") and selecao and nome:
            st.session_state.grupos_dict[nome] = selecao
            st.rerun()
    if st.session_state.grupos_dict:
        remover = []
        for g, l in st.session_state.grupos_dict.items():
            if st.sidebar.button(f"Remover {g}", key=f"del_{g}"): remover.append(g)
        if remover:
            for r in remover: del st.session_state.grupos_dict[r]
            st.rerun()

    def definir_motivo(m):
        for g, l in st.session_state.grupos_dict.items():
            if m in l: return g
        return m
    df_ret_filtrado['Motivo_Analise'] = df_ret_filtrado[col_motivo].apply(definir_motivo)

    # --- CÁLCULOS KPI GERAL ---
    df_p_agg = df_prod.rename(columns={col_equipe_p: 'Equipe'})
    df_r_agg = df_ret_filtrado.rename(columns={col_equipe_r: 'Equipe'})

    prod_agg = df_p_agg.groupby(['Linha', 'Equipe'])['metragem_real'].sum().reset_index().rename(columns={'metragem_real': 'M2_Produzido'})
    ret_agg = df_r_agg.groupby(['Linha', 'Equipe'])['m2_real'].sum().reset_index().rename(columns={'m2_real': 'M2_Retido'})
    df_final = pd.merge(prod_agg, ret_agg, on=['Linha', 'Equipe'], how='left').fillna(0)
    
    df_final['Meta_M2'] = df_final['M2_Produzido'] * (META_PCT / 100)
    
    # NOVA LÓGICA SALDO: Meta - Realizado
    df_final['Saldo_M2'] = df_final['Meta_M2'] - df_final['M2_Retido']
    
    df_final['% Realizado'] = (df_final['M2_Retido'] / df_final['M2_Produzido']) * 100

    df_l45_completo = adicionar_linha_geral(df_final, 'Linha 4 e 5', META_PCT)
    df_l6_completo = adicionar_linha_geral(df_final, 'Linha 6', META_PCT)

    def definir_status_meta(valor_pct):
        return 'Dentro da Meta (Verde)' if valor_pct <= META_PCT else 'Fora da Meta (Vermelho)'

    if df_l45_completo is not None: df_l45_completo['Status'] = df_l45_completo['% Realizado'].apply(definir_status_meta)
    if df_l6_completo is not None: df_l6_completo['Status'] = df_l6_completo['% Realizado'].apply(definir_status_meta)

    # Tabela Final
    df_tabela_final = pd.concat([df_l45_completo, df_l6_completo], ignore_index=True)
    if not df_tabela_final.empty:
        # Ajuste para exibir Saldo_M2
        cols_exist = [c for c in df_tabela_final.columns if c in ['Linha', 'Equipe', 'M2_Produzido', 'Meta_M2', 'M2_Retido', 'Saldo_M2', '% Realizado']]
        df_exibicao = df_tabela_final[cols_exist].copy()
    else: df_exibicao = pd.DataFrame()

    # --- DASHBOARD ---
    tab1, tab2, tab3 = st.tabs(["📊 Resultados Consolidados", "🔍 Análise por Motivo", "💾 Dados Brutos"])

    with tab1:
        st.subheader("📈 Indicadores Gerais")
        c1, c2 = st.columns(2)
        with c1:
            st.info("**Linha 4 e 5**")
            if df_l45_completo is not None and not df_l45_completo.empty:
                row = df_l45_completo[df_l45_completo['Equipe'] == 'Média Geral']
                if not row.empty:
                    val = row['% Realizado'].values[0]
                    st.metric("Perda Média", f"{val:.2f}%", delta=f"{val-META_PCT:.2f}% vs Meta", delta_color="inverse")
        with c2:
            st.info("**Linha 6**")
            if df_l6_completo is not None and not df_l6_completo.empty:
                row = df_l6_completo[df_l6_completo['Equipe'] == 'Média Geral']
                if not row.empty:
                    val = row['% Realizado'].values[0]
                    st.metric("Perda Média", f"{val:.2f}%", delta=f"{val-META_PCT:.2f}% vs Meta", delta_color="inverse")

        st.markdown("---")
        st.subheader(f"📊 Performance Total por Equipe (Meta: {META_PCT}%)")
        col_g1, col_g2 = st.columns(2)
        mapa_cores = {'Dentro da Meta (Verde)': '#27AE60', 'Fora da Meta (Vermelho)': '#E74C3C'}

        with col_g1:
            if df_l45_completo is not None and not df_l45_completo.empty:
                fig1 = go.Figure()
                fig1.add_trace(go.Bar(
                    x=df_l45_completo['Equipe'], y=df_l45_completo['% Realizado'],
                    marker_color=[mapa_cores.get(s, '#333') for s in df_l45_completo['Status']],
                    text=[f"{v:.2f}" for v in df_l45_completo['% Realizado']], 
                    textposition='inside', insidetextanchor='middle'
                ))
                fig1.add_hline(y=META_PCT, line_dash="dot", line_color="black")
                fig1.update_layout(title="L4/L5: % ", template=TEMPLATE_GRAFICO, xaxis_title="Equipe", yaxis_title="%")
                st.plotly_chart(fig1, use_container_width=True)

        with col_g2:
            if df_l6_completo is not None and not df_l6_completo.empty:
                fig2 = go.Figure()
                fig2.add_trace(go.Bar(
                    x=df_l6_completo['Equipe'], y=df_l6_completo['% Realizado'],
                    marker_color=[mapa_cores.get(s, '#333') for s in df_l6_completo['Status']],
                    text=[f"{v:.2f}" for v in df_l6_completo['% Realizado']],
                    textposition='inside', insidetextanchor='middle'
                ))
                fig2.add_hline(y=META_PCT, line_dash="dot", line_color="black")
                fig2.update_layout(title="L6: % ", template=TEMPLATE_GRAFICO, xaxis_title="Equipe", yaxis_title="%")
                st.plotly_chart(fig2, use_container_width=True)

        st.markdown("---")
        st.subheader("📅 Retido em M²")
        st.caption(f"Meta definida: {META_PCT}%")
        
        if 'mes_ano' in df_p_agg.columns:
            col_t1, col_t2 = st.columns(2)
            df_prod_renamed = df_prod.rename(columns={col_equipe_p: 'Equipe'})
            df_ret_renamed = df_ret_filtrado.rename(columns={col_equipe_r: 'Equipe'})
            
            with col_t1:
                fig_t1 = criar_grafico_evolucao_com_geral(df_prod_renamed, df_ret_renamed, 'Linha 4 e 5', META_PCT)
                if fig_t1: st.plotly_chart(fig_t1, use_container_width=True)
                else: st.info("Sem dados mensais para L4/L5")
            
            with col_t2:
                fig_t2 = criar_grafico_evolucao_com_geral(df_prod_renamed, df_ret_renamed, 'Linha 6', META_PCT)
                if fig_t2: st.plotly_chart(fig_t2, use_container_width=True)
                else: st.info("Sem dados mensais para L6")
        else: st.warning("Coluna de data não encontrada.")

        st.markdown("---")
        st.subheader("📋 Detalhamento")
        fig_tabela = criar_tabela_grafica(df_exibicao, META_PCT)
        if fig_tabela: st.plotly_chart(fig_tabela, use_container_width=True)

        st.markdown("---")
        st.subheader("🏆 Top Causas de Retenção (M²)")
        c1, c2 = st.columns(2)
        
        df_m_l45 = df_ret_filtrado[df_ret_filtrado['Linha'] == 'Linha 4 e 5']
        if not df_m_l45.empty:
            top_l45 = df_m_l45.groupby('Motivo_Analise')['m2_real'].sum().sort_values(ascending=False).head(10).reset_index()
            fig_top1 = px.bar(top_l45, y='Motivo_Analise', x='m2_real', orientation='h', title="Top 10 - L4/L5", template=TEMPLATE_GRAFICO, text_auto='.2f')
            fig_top1.update_traces(textposition='outside')
            
            # <--- CORREÇÃO AQUI: Expandir eixo X para caber o texto
            max_val_l45 = top_l45['m2_real'].max()
            fig_top1.update_layout(
                yaxis={'categoryorder':'total ascending'},
                xaxis=dict(range=[0, max_val_l45 * 1.25]) # +25% de margem
            )
            c1.plotly_chart(fig_top1, use_container_width=True)
            
        df_m_l6 = df_ret_filtrado[df_ret_filtrado['Linha'] == 'Linha 6']
        if not df_m_l6.empty:
            top_l6 = df_m_l6.groupby('Motivo_Analise')['m2_real'].sum().sort_values(ascending=False).head(10).reset_index()
            fig_top2 = px.bar(top_l6, y='Motivo_Analise', x='m2_real', orientation='h', title="Top 10 - L6", template=TEMPLATE_GRAFICO, text_auto='.2f')
            fig_top2.update_traces(textposition='outside')
            
            # <--- CORREÇÃO AQUI: Expandir eixo X para caber o texto
            max_val_l6 = top_l6['m2_real'].max()
            fig_top2.update_layout(
                yaxis={'categoryorder':'total ascending'},
                xaxis=dict(range=[0, max_val_l6 * 1.25]) # +25% de margem
            )
            c2.plotly_chart(fig_top2, use_container_width=True)

        # --- NOVA SEÇÃO: DETALHES DE FILTROS E GRUPOS ---
        st.markdown("---")
        with st.expander("📝 Detalhes de Agrupamentos e Exclusões Ativos", expanded=True):
            col_info1, col_info2 = st.columns(2)
            
            with col_info1:
                st.markdown("**🚫 Motivos Excluídos**")
                if motivos_excluir:
                    for m in motivos_excluir:
                        st.text(f"• {m}")
                else:
                    st.caption("Nenhum motivo excluído da análise.")
            
            with col_info2:
                st.markdown("**🔄 Agrupamentos Realizados**")
                if st.session_state.grupos_dict:
                    for nome_grupo, lista_originais in st.session_state.grupos_dict.items():
                        st.markdown(f"**{nome_grupo}** agrupa:")
                        st.caption(f"{', '.join(lista_originais)}")
                else:
                    st.caption("Nenhum agrupamento ativo.")

    with tab2:
        if motivo_alvo and motivo_alvo != "(Selecione um motivo)":
            st.subheader(f"🔎 Análise Detalhada: {motivo_alvo}")
            
            # --- PREPARAÇÃO DOS DADOS ---
            df_spec = df_ret[df_ret[col_motivo] == motivo_alvo].copy()
            todas_equipes = pd.DataFrame({'Equipe': sorted(df_prod[col_equipe_p].unique())})
            
            spec_agg = df_spec.groupby(col_equipe_r)['m2_real'].sum().reset_index().rename(columns={col_equipe_r: 'Equipe', 'm2_real': 'M2_Retido'})
            spec_count = df_spec.groupby(col_equipe_r).size().reset_index(name='Qtd_Ocorrencias')
            
            spec_final = pd.merge(todas_equipes, spec_agg, on='Equipe', how='left').fillna(0)
            spec_final = pd.merge(spec_final, spec_count, on='Equipe', how='left').fillna(0)
            
            # Agregação por Linha
            df_spec['Linha'] = df_spec[col_forno_r].apply(mapear_linha)
            spec_linha = df_spec.groupby('Linha').size().reset_index(name='Qtd_Ocorrencias')
            todas_linhas = pd.DataFrame({'Linha': ['Linha 4 e 5', 'Linha 6']})
            spec_linha_final = pd.merge(todas_linhas, spec_linha, on='Linha', how='left').fillna(0)

            # --- GRÁFICOS ---
            col_spec1, col_spec2 = st.columns(2)
            
            # 1. Gráfico M² por Equipe
            with col_spec1:
                # Cor M2 se ativado
                if USAR_META_M2:
                    spec_final['Cor_M2'] = spec_final['M2_Retido'].apply(lambda x: '#27AE60' if x <= META_ABSOLUTA_M2 else '#E74C3C')
                else:
                    spec_final['Cor_M2'] = '#2E86C1' # Azul padrão

                fig_m2 = go.Figure()
                fig_m2.add_trace(go.Bar(
                    x=spec_final['Equipe'].astype(str), y=spec_final['M2_Retido'], marker_color=spec_final['Cor_M2'],
                    text=[f"{v:,.2f}" for v in spec_final['M2_Retido']], textposition='auto', name='M²'
                ))
                if USAR_META_M2:
                    fig_m2.add_hline(y=META_ABSOLUTA_M2, line_dash="dash", line_color="black", annotation_text=f"Meta: {META_ABSOLUTA_M2} m²")
                
                fig_m2.update_layout(title=f"Metragem por Equipe", xaxis_title="Equipe", template=TEMPLATE_GRAFICO)
                st.plotly_chart(fig_m2, use_container_width=True)

            # 2. Gráfico Frequência por Equipe
            with col_spec2:
                if USAR_META_FREQ:
                    spec_final['Cor_Freq'] = spec_final['Qtd_Ocorrencias'].apply(lambda x: '#27AE60' if x <= META_FREQ_QTD else '#E74C3C')
                else:
                    spec_final['Cor_Freq'] = "#03335A"

                fig_count = go.Figure()
                fig_count.add_trace(go.Bar(
                    x=spec_final['Equipe'].astype(str), y=spec_final['Qtd_Ocorrencias'],
                    marker_color=spec_final['Cor_Freq'],
                    text=spec_final['Qtd_Ocorrencias'], textposition='auto', name='Qtd'
                ))
                if USAR_META_FREQ:
                    fig_count.add_hline(y=META_FREQ_QTD, line_dash="dash", line_color="black", annotation_text=f"Meta: {META_FREQ_QTD}")
                
                fig_count.update_layout(title=f"Quantidade de Ocorrências por Equipe", xaxis_title="Equipe", template=TEMPLATE_GRAFICO)
                st.plotly_chart(fig_count, use_container_width=True)

            st.markdown("---")
            
            # 3. Gráfico Incidência por Linha
            st.subheader("📍 Onde acontece mais? (Incidência por Linha)")
            
            if USAR_META_FREQ:
                spec_linha_final['Cor_Linha'] = spec_linha_final['Qtd_Ocorrencias'].apply(lambda x: '#27AE60' if x <= META_FREQ_QTD else '#E74C3C')
            else:
                spec_linha_final['Cor_Linha'] = '#2E86C1' 

            fig_linha = go.Figure()
            fig_linha.add_trace(go.Bar(
                x=spec_linha_final['Linha'], y=spec_linha_final['Qtd_Ocorrencias'],
                marker_color=spec_linha_final['Cor_Linha'],
                text=spec_linha_final['Qtd_Ocorrencias'], textposition='auto'
            ))
            if USAR_META_FREQ:
                fig_linha.add_hline(y=META_FREQ_QTD, line_dash="dash", line_color="black", annotation_text=f"Meta: {META_FREQ_QTD}")

            fig_linha.update_layout(title="Total de Ocorrências por Linha de Produção", template=TEMPLATE_GRAFICO)
            st.plotly_chart(fig_linha, use_container_width=True)

            st.markdown("---")
            st.write("📋 **Dados Consolidados:**")
            st.dataframe(spec_final[['Equipe', 'M2_Retido', 'Qtd_Ocorrencias']].style.format({"M2_Retido": "{:.2f}", "Qtd_Ocorrencias": "{:.0f}"}))
            
        else:
            st.info("👈 Selecione um motivo na barra lateral (Seção 3) para visualizar esta análise.")

    with tab3:
        st.dataframe(df_tabela_final, use_container_width=True)
        st.download_button("📥 Baixar Excel", data=convert_df_to_excel(df_tabela_final), file_name="relatorio_completo.xlsx")
else:
    st.info("Aguardando upload dos arquivos.")